...
export function MuscleDiagram(props: { principale: MuscleGroup[]; secundare?: MuscleGroup[]; marime?: number }) {
  ...
  const f = (g: MuscleGroup) => (on(g) ? 'var(--rosu, #D0342C)' : sec(g) ? '#f0a89f' : 'var(--panou, #fff)');
    ...
    <svg height={h} viewBox="0 0 220 260" role="img" aria-label="Grupele musculare lucrate" style={{ maxWidth: '100%' }}>
      <g stroke={contur} strokeWidth="2" strokeLinejoin="round">
        ...
        <g transform="translate(10,6)">
          <circle cx="45" cy="16" r="13" fill="var(--panou,#fff)" />
          <rect x="38" y="29" width="14" height="8" fill="var(--panou,#fff)" />
          {/* umeri */}
          ...
          {/* tibie/gambe față */}
          <rect x="28" y="140" width="13" height="38" rx="6" fill="var(--panou,#fff)" />
          <rect x="49" y="140" width="13" height="38" rx="6" fill="var(--panou,#fff)" />
        ...
        {(on('cardio') || sec('cardio')) && (
          <path
            ...
            fill={on('cardio') ? 'var(--rosu,#D0342C)' : '#f0a89f'}
          >
